...
export const Business: React.FC = () => {
    ...
    useEffect(() => {
        ...
        api.get<Assessment[]>(`/assessments/company/${params.id}`).then(
            (response) => {
    ...
    const handleFavoriteCompany = useCallback(async () => {
        if (company.id) {
            if (!companyFavorited) {
                ...
                await api.patch(
                    `/companies/favorites/unfavorite/${company.id}`
                );
    ...
    const handleAddAssessments = useCallback(
        async (newAssessment: Assessment) => {
            ...
            await api
                .get<Assessment[]>(`/assessments/company/${params.id}`)
                .then((response) => {
                    setAssessments(response.data);
                });
        },
    ...
    const convertToPictureProps = useCallback(
        (imageCompany: ImageCompany): PictureProps => {
            return {
    ...
    return (
        <div className="flex flex-col">
            ...
            <BusinessHeader company={company} />
            <div className="px-24 py-12">
                <div className="flex flex-col gap-4 mobile:gap-4">
                    ...
                    {company.services && company.services.length > 0 && (
                        <div className="flex flex-col">
                            <span className="font-inter font-semibold text-gray-700 mb-2">
                                Serviços oferecidos
                            </span>
                    ...
                    {company.Address && company.physical_localization ? (
                        ...
                        company.Address && (
                            <Paragraph
                                ...
                                ${company.Address.city} -
                                ${company.Address.state}`}
                                link={`https://www.google.com/maps/search/${company.Address.cep || company.Address.city
                                    }/@-19.5406953,-43.4148527,11z/data=!3m1!4b1`}
                            />
                        )
                    )}
                    ...
                    {company.ImageCompany &&
                        company.ImageCompany.length > 0 && (
                            <div className="flex flex-col gap-2">
                                ...
                                {showFullScreenImages && (
                                    <Gallery
                                        images={company.ImageCompany.map(
                                            convertToPictureProps
                                        )}
                                ...
                                <div className="flex flex-col mt-[2.25rem]">
                                    <div className="flex flex-col gap-[0.375rem]">
                                        ...
                                        <p className="font-inter font-light text-xs">
                                            {assessments.length} comentário(s)
                                        </p>
                                    </div>
                                    {assessments
                                        .slice(-quantComments)
